 ...
 * By default, {@link WebErrorHandlers} use the {@link LastResortWebErrorHandler} as the
...
public class ErrorsAutoConfiguration {
    ...
    private static final String ERROR_CODE_MESSAGE = "error_code";
    ...
    public ErrorsAutoConfiguration(ResourceBundleMessageSource source,
    ...
    public void postConstruct(){
        String[] errorCodeProperties = errorsProperties.getErrorCodeProperties();
        if (ArrayUtil.isNotEmpty(errorCodeProperties)){
            source.addBasenames(errorCodeProperties);
        }
        source.addBasenames(ERROR_CODE_MESSAGE);
        errorsProperties.setAddFingerprint(true);
    }
     ...
     * {@link HttpError} to Spring's {@link ErrorAttributes} abstraction.
     *
     * @param errorsProperties Configuration properties bean.
...
//    }
     ...
     */
    @Bean
    @ConditionalOnMissingBean(WebErrorHandlerPostProcessor.class)
    public LoggingErrorWebErrorHandlerPostProcessor processor(){
        return new LoggingErrorWebErrorHandlerPostProcessor();
    }
