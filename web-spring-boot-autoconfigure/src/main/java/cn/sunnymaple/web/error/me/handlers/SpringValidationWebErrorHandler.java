...
public class SpringValidationWebErrorHandler implements WebErrorHandler {
    ...
    public HandledException handle(Throwable exception) {
        ...
        return bindingResult.getAllErrors()
            ...
            .collect(collectingAndThen(
                toMap(this::userTip, this::arguments, (value1, value2) -> value1), m -> new HandledException(A0400, HttpStatus.BAD_REQUEST,m.keySet(), dropEmptyValues(m))
            ));
    }
    ...
    private String userTip(ObjectError error) {
        String userTip = null;
        try {
            userTip = error.unwrap(ConstraintViolation.class).getMessageTemplate();
        } catch (Exception ignored) {
        }

        if (userTip == null) {
            try {
                userTip = TypeMismatchWebErrorHandler.getErrorCode(error.unwrap(TypeMismatchException.class));
            } catch (Exception ignored) {
            }
        }
        return userTip == null ? BINDING_FAILURE : userTip;
    }
