...
import cn.sunnymaple.web.error.me.message.TemplateAwareMessageSource;
import cn.sunnymaple.web.error.me.conf.ErrorsProperties;
import cn.sunnymaple.web.error.me.HttpError.CodedMessage;
...
public class WebErrorHandlers {
    ...
    /**
     * This is the fall back error handler which will be used when all other {@link WebErrorHandler}
    ...
    private List<CodedMessage> translateErrors(HandledException handled, Locale locale) {
        return handled
            .getUserTips()
            .stream()
            .map(userTip -> withMessage(userTip,handled.getErrorCode(), getArgumentsFor(handled, userTip), locale))
            .collect(toList());
    }

    private CodedMessage withMessage(String userTip,String errorCode, List<Argument> arguments, Locale locale) {
        try {
            String message = messageSource.interpolate(userTip, errorCode,arguments, locale);
            return new CodedMessage(errorCode, message, arguments);
        } catch (Exception e) {
            return new CodedMessage(errorCode, null, arguments);
        }
    }

    /**
     * 获取处理异常的{@link WebErrorHandler}
     * @param exception 异常
     * @return
     */
    ...
    private List<Argument> getArgumentsFor(HandledException handled, String userTip) {
        return handled.getArguments().getOrDefault(userTip, emptyList());
    }
}
